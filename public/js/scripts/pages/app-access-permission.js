...
$(function () {
  ...
  var dt_permissions_table = $('.datatables-permissions'),
    dt_permission,
    userList = 'app-user-list.html';
 // Datatables for Permissions
  if (dt_permissions_table.length) {
    var dt_authtype= dt_permissions_table.DataTable({
      ajax: 'http://local.laraveladmin/su/data/permissions-list',
      columns: [
        { data: 'id' },
        { data: 'id' },
        ...
        },
        {
          ...
          render: function (data, type, full, meta) {
            return (
              ...
              data +'" name="deleteids[]"/><label class="form-check-label" for="checkbox' +
              data +
              '"></label></div>'
            );
          },
          ...
          render: function (data, type, full, meta) {
            ...
            var roleBadgeObj = {
              Superadmin:
                '<a href="' +
                userList +
                '" class="me-50"><span class="badge rounded-pill badge-light-primary">Superadmin</span></a>',
              Executive:
                '<a href="' +
                userList +
                '" class="me-50"><span class="badge rounded-pill badge-light-warning">Executive</span></a>',
              Users:
                '<a href="' +
                userList +
        ...
        },
        {
          ...
          render: function (data, type, full, meta) {
            return (
              ...
              '</div>' +
              '</div>' +
              '<a href="http://local.laraveladmin/su/authentication-types/'+full['id']+'/edit" class="item-edit">' +
  ...
  //     ajax: 'http://local.laraveladmin/su/data/permissions-list', // JSON file to add data
  //     columns: [
      ...
      initComplete: function () {
        ...
        this.api()
          ...
          .every(function () {
            var column = this;
            var select = $(
              '<select id="UserRole" class="form-select text-capitalize"><option value=""> Select Role </option><option value="Superadmin" class="text-capitalize">Superadmin</option><option value="Executive" class="text-capitalize">Executive</option><option value="Users" class="text-capitalize">users</option><option value="Support" class="text-capitalize">Support</option><option value="Restricted" class="text-capitalize">Restricted User</option></select>'
            )
              .appendTo('.user_role')
              .on('change', function () {
